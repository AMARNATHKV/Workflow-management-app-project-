...
router.post("/register", async (req, res) => {
  try {
    const userDetails = req.body;
    const { username, password, email, userType } = userDetails;

    if (userType === "manager") {
      const existingManager = await User.findOne({ userType: "manager" });
      if (existingManager) {
        return res.status(400).json({ error: "A manager already exists" });
      }
    }
